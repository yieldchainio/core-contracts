/**
 * Lending adapter for Radiant V2
 * It's a fork of AaveV3 but with some very minor changes
 */
...
import {IRadiantV2PoolAddressesProvider} from "src/interfaces/IRadiantV2PoolAddressesProvider.sol";

contract RadiantLendingAdapter is
    ...
    ERC20Utils
{
    ...
    function _getDataProviderAndPool(
        ...
        IRadiantV2PoolAddressesProvider poolAddressesProvider = IRadiantV2PoolAddressesProvider(
                poolRegistry.getAddressesProvidersList()[0]
            );

        pool = IPool(poolAddressesProvider.getLendingPool());
