...
contract YCParsers is YieldchainTypes, Interpreters {
    ...
    function interpretCommandsAndEncodeChunck(
        ...
        for (uint256 i = 0; i < ycCommands.length; i++) {
            if (ycCommands[i][1] == REF_VAR_FLAG) ++dynamicVarsAmt;
        }
    ...
    function interpretCommandsArr(
         ...
         *
         * If the return typeflag is a COMMANDS_REF_ARR_FLAG,
         ...
         *
         * Do note that, in neither cases, we append a memory pointer. The calldata builder expects
         * a "naked" value (length (optionally) + data), and manages the offset pointers itself.
         */
